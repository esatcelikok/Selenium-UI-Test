...
public class Utility extends FlowsLibrary {
    ...
    public void comboboxwait(WebElement element5){
        WebDriverWait wait = new WebDriverWait(Driver.getDriver(), Duration.ofSeconds(20));
        wait.until(bekleme  ->
                "true".equals(element5.getAttribute("expanded")));
    }

    public boolean isElementPresent(By locator) {
        try {
            WebDriverWait wait = new WebDriverWait(Driver.getDriver(),Duration.ofSeconds(20));  // 10 saniye bekle
            wait.until(ExpectedConditions.presenceOfElementLocated(locator));
            return true;
